...
// Pastikan kedua parameter (id_detail dan id_pesanan) ditangkap dari URL
if (isset($_GET['id']) && isset($_GET['id_pesanan'])) {
    $id_detail = $_GET['id'];
    $id_pesanan = $_GET['id_pesanan'];
    
    try {
        $pdo->beginTransaction(); // Mulai transaksi untuk keamanan data

        // 1. Hapus item menu dari rincian (keranjang)
        $sql = "DELETE FROM detail_pesanan WHERE id_detail = ?";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$id_detail]);

        // 2. OTOMATISASI: Update total di tabel pesanan utama!
